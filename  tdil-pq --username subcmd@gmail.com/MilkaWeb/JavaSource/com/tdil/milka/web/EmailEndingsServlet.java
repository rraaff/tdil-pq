...
public class EmailEndingsServlet extends HttpServlet {
	...
	private void doService(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		List<EmailEndingsValueObject> answer = EmailEndingsUtils.getEmailEndings();
		...
		for (EmailEndingsValueObject emailEndingsValueObject : answer) {
			...
			out.append(emailEndingsValueObject.getName());
			out.append(CDATA_END_TAG).append("</name>");
			out.append("<title>").append(CDATA_START_TAG);
			out.append(emailEndingsValueObject.getTitle());
