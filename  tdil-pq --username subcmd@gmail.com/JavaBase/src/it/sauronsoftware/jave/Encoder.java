...
public class Encoder {
	private static String ERROR_MESSAGE_CONVERT = "Error to convert file";
	/**
	...
	private static final Pattern FRAME_RATE_PATTERN = Pattern.compile(
			...
			"([\\d.]+)\\s+(?:fps|tb\\w?)", Pattern.CASE_INSENSITIVE);
	...
	private static final Pattern CHANNELS_PATTERN = Pattern.compile(
			...
			"(mono|stereo|[\\d\\.]+ channels|[\\d\\.]+)", Pattern.CASE_INSENSITIVE);
	...
	private KeyFrameMaker keyFrameMaker;
	
	private StringBuffer libraryOutput;
		...
		try {
			...
			while (true) {
				...
				if (step == 0) {
					...
					if (m.matches()) {
						/*
						 * Cuando estamos en Step 1 esperamos que la linea empiece con " Duration: 00:00:00.0"
						 * Si el video esta mal lo que viene es " Duration: N/A"
						 * El problema esta cuando el video tiene metadata y esta viene entre el Header del Codec y Duration
						 */
						...
						Pattern pTemp = Pattern.compile("^\\s*Duration:.*$", Pattern.CASE_INSENSITIVE);
						if (pTemp.matcher(line).matches()) {
							step = 3;
						}
					}
				} else if (step == 2) {
								...
								if (i == 0) {
									...
									if (!parsed && m2.find()) {
										...
										if ("mono".equalsIgnoreCase(ms)) {
											...
											audio.setChannels(2);
											parsed = true;
										} else if (ms.indexOf("channels") != -1) {
											String ch = ms.substring(0, ms.indexOf("channels")).trim();
											if ("1".equals(ch)) {
												audio.setChannels(1);
												parsed = true;
											} else if ("2".equals(ch)) {
												audio.setChannels(2);
												parsed = true;
											} else if ("5.1".equals(ch)) {
												audio.setChannels(6);
												parsed = true;
											}
										} else if ("5.1".equals(ms)) {
											audio.setChannels(6);
											parsed = true;
										}
									}
	 ...
	 */
	public void encode(File source, File target, EncodingAttributes attributes, Integer percentOfDiff)
			throws IllegalArgumentException, InputFormatException,
			EncoderException {
		encode(source, target, attributes, percentOfDiff, null);
	}
	...
	public void encode(File source, File target, EncodingAttributes attributes, Integer percentOfDiff,
		...
		Integer vframesAttribute = attributes.getFrames();
		...
		if (vframesAttribute != null) {
			ffmpeg.addArgument("-vframes");
			ffmpeg.addArgument(String.valueOf(vframesAttribute.intValue()));
		}
		ffmpeg.addArgument("-i");
		ffmpeg.addArgument(source.getAbsolutePath());
		...
		try {
			...
			this.setLibraryOutput(sb);
			if ( hasErrorWhileDecoding) {
				MultimediaInfo sourceInfo = this.getInfo(source);
				MultimediaInfo targetInfo = this.getInfo(target);
				if (sourceInfo.getAudio() != null && targetInfo.getAudio() == null) {
					// If original has audio and converted does not, it is an error
					throw new EncoderException(ERROR_MESSAGE_CONVERT);
				}
				if (sourceInfo.getAudio() == null && targetInfo.getAudio() != null) {
					// If original does not have audio and converted has, it is an error
					throw new EncoderException(ERROR_MESSAGE_CONVERT);
				}
				if (sourceInfo.getVideo() != null && targetInfo.getVideo() == null) {
					// If original has video and converted does not, it is an error
					throw new EncoderException(ERROR_MESSAGE_CONVERT);
				}
				if (sourceInfo.getVideo() == null && targetInfo.getVideo() != null) {
					// If original does not have video and converted has, it is an error
					throw new EncoderException(ERROR_MESSAGE_CONVERT);
				}
				if (sourceInfo.getDuration() > 0 && targetInfo.getDuration() > 0) {
					long diff = (sourceInfo.getDuration() - targetInfo.getDuration());
					if((((float)Math.abs(diff)/(float)sourceInfo.getDuration())*100) > percentOfDiff ){
						throw new EncoderException(ERROR_MESSAGE_CONVERT);
					}
				}
				if (sourceInfo.getDuration() == -1 && targetInfo.getDuration() == -1) {
					//Input file cannot really be converted
					throw new EncoderException(ERROR_MESSAGE_CONVERT);
				}
			}
...
//			}
		...
		try{
		//Generates keyframe
			if(this.keyFrameMaker != null){
				this.keyFrameMaker.setFileName(target.getAbsolutePath());
				this.keyFrameMaker.execute();
			}
		} catch (Exception e) {
			throw new EncoderException(e);
		}
	...
	public void setAudioKeyFrame(){
		this.keyFrameMaker = new KeyFrameMaker();
		this.keyFrameMaker.generateAudioParameters();
	}
	
	public void setVideoKeyFrame(){
		this.keyFrameMaker = new KeyFrameMaker();
		this.keyFrameMaker.generateVideoParameters();
	}

	public StringBuffer getLibraryOutput() {
		return libraryOutput;
	}

	public void setLibraryOutput(StringBuffer libraryOutput) {
		this.libraryOutput = libraryOutput;
	}
}