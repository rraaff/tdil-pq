...
class FFMPEGExecutor {
	...
	public void execute() throws IOException {
		...
		if (System.getProperty("os.name").toUpperCase().indexOf("WINDOWS") != -1) {
			...
			cmd = new String[argsSize + 4];
			cmd[0] = "nice";
			cmd[1] = "-n";
			cmd[2] = "10";
			firstParamIndex = 3;
		}
		cmd[firstParamIndex] = ffmpegExecutablePath;
		for (int i = firstParamIndex, j = 0; i < argsSize + firstParamIndex; i++, j++) {
			cmd[i + 1] = (String) args.get(j);
		}
