...
public class ClientStatusHelper {
	...
	public static String getStatusFor(Client profesional) {
		if (profesional.getStatus().equals(ClientStatus.EMAIL_VALIDATION_PENDING)) {
			return ApplicationResources.getMessage(EMAIL_NOT_VALIDATED);
		} else {
			if (profesional.getStatus().equals(ClientStatus.BLOCKED)) {
				return ApplicationResources.getMessage(BLOCKED);
			} else {
				if (profesional.getStatus().equals(ClientStatus.APPROVED)) {
