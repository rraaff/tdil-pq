			...
			<% 	int milkaPhotoIndex = 0;
				for (MilkaPhotoValueObject mfvo : toDisplay) { %>
					<% if (milkaPhotoIndex == 0) { %>
						<li class="f-grande"><a href="./download.st?id=<%=mfvo.getIdApprovedData()%>&type=PUBLIC&ext=<%=mfvo.getExtApprovedData()%>" title="tu foto milka" rel="lightbox[pm]" button="<%=mfvo.getIdClickCounter()%>-<%=MeltButton.meltButtonCount(mfvo.getIdClickCounter())%>" class="activo"><img src="./download.st?id=<%=mfvo.getIdApprovedData()%>&type=PUBLIC&ext=<%=mfvo.getExtApprovedData()%>" width="212" height="157" alt="foto1" /></a></li>
					<% } else { %>
...
<div class="hide">
	<% for (MilkaPhotoValueObject mfvo : hiddenDisplay) { %>
		<a href="./download.st?id=<%=mfvo.getIdApprovedData()%>&type=PUBLIC&ext=<%=mfvo.getExtApprovedData()%>" rel="lightbox[pm]" title="tu foto milka" button="<%=mfvo.getIdClickCounter()%>-<%=MeltButton.meltButtonCount(mfvo.getIdClickCounter())%>" class="activo">a</a>
	<% } %>
</div>
