...
import com.tdil.chivas.co.struts.forms.beans.OptIn;
import com.tdil.chivas.co.utils.ThalamusWebUtils;
...
public class RegisterForm extends AbstractForm implements RefreshableForm {
	...
	private void createDynamicFields() {
		...
		for (FieldDescription fd : this.personFields.getDynamicFields()) {
			dynamicFields.add(new DynamicField(fd));
		}
	}
	...
	private JSONObject getPersonJSON() {
		...
		for (DynamicField df : this.getDynamicFields()) {
			if (df.getFieldDescription().isSubfield()) {
				if (profile.containsKey(df.getFieldDescription().getParent().getName())) {
					JSONObject parent = profile.getJSONObject(df.getFieldDescription().getParent().getName());
					Object value = df.getObjectValue();
					if(value != null) {
						parent.put(df.getFieldDescription().getName(), value);
					}
				} else {
					JSONObject parent = new JSONObject();
					Object value = df.getObjectValue();
					if(value != null) {
						parent.put(df.getFieldDescription().getName(), df.getObjectValue());
					}
					profile.put(df.getFieldDescription().getParent().getName(), parent);
				}
			} else {
				Object value = df.getObjectValue();
				if(value != null) {
					profile.put(df.getFieldDescription().getName(), df.getObjectValue());
				}
			}
		}
	...
	public void takeInitialData(TokenHolder tokenHolder) throws HttpStatusException, InvalidResponseException, CommunicationException, UnauthorizedException {
		...
		for (DynamicField df : this.getDynamicFields()) {
			df.takeInitialValueFrom(profile);
		}
	...
	public DynamicField getDynamicField(int index) {
		return getDynamicFields().get(index);
	}
	
	public void setDynamicField(int index, DynamicField dynamicField) {
		getDynamicFields().set(index, dynamicField);
	}
}
