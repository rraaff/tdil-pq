...
public class TestLoJack {
	...
	public static void main(String[] args) {
		...
		// LIGHTS
		// list
		given().cookie(cookie).expect().body("lights.idLuz", hasItems(1, 2)).when().get("http://localhost:8180/LoJackWeb/rest/lights/list");
		// Rename
		given().cookie(cookie).param("description", newName).expect().body("ok", equalTo(true)).when().get("http://localhost:8180/LoJackWeb/rest/lights/{idEntidad}/{idLuz}/rename",70843, 1);
		given().cookie(cookie).expect().body("lights.description", hasItems(newName)).when().get("http://localhost:8180/LoJackWeb/rest/lights/list");
		// get log
		given().cookie(cookie).expect().body("logs.action", hasItems("ACTIVAR")).when().get("http://localhost:8180/LoJackWeb/rest/lights/{idEntidad}/{idLuz}/log",70843,1);
		
		// activate
		given().cookie(cookie).expect().body("accepted", equalTo(true)).when().get("http://localhost:8180/LoJackWeb/rest/lights/{idEntidad}/{idLuz}/activate",70843,1);
		// deactivate
		given().cookie(cookie).expect().body("accepted", equalTo(true)).when().get("http://localhost:8180/LoJackWeb/rest/lights/{idEntidad}/{idLuz}/deactivate",70843,1);
		// randomOn
		given().cookie(cookie).expect().body("accepted", equalTo(true)).when().get("http://localhost:8180/LoJackWeb/rest/lights/{idEntidad}/{idLuz}/randomOn",70843,1);
		// randomOff
		given().cookie(cookie).expect().body("accepted", equalTo(true)).when().get("http://localhost:8180/LoJackWeb/rest/lights/{idEntidad}/{idLuz}/randomOff",70843,1);
