...
import com.tdil.ljpeugeot.feeds.dealer.DealerImportRecord;
...
public class VLUImportThread extends Thread {
	...
	private static final class InsertVLUData implements TransactionalAction {
		...
		public InsertVLUData(int importId, DealerImportRecord importRecord) {
			super();
		...
		public void executeInTransaction() throws SQLException {
			...
			preparedStatement.setString(1, this.importRecord.getState());
			preparedStatement.setString(2, this.importRecord.getCity());
			if (StringUtils.isEmpty(this.importRecord.getName())) {
				preparedStatement.setString(3, null);
			} else {
				preparedStatement.setString(3, this.importRecord.getName());
			}
...
//			DAOManager.getVLUImportDAO().incrementProcessed(this.importId);
	...
	private void processImport(VLUImport imp) {
		...
		try {
			...
			try {
				...
				DealerImportRecord importRecord;
				int commitCount = 1;
				while ((importRecord = beanReader.read(DealerImportRecord.class, header, processors)) != null) {
	...
	private void processImport(CsvBeanReader beanReader, VLUImport vluImport, DealerImportRecord importRecord, 
		...
		insertVlu.setString(1, importRecord.getState());
		insertVlu.setString(2, importRecord.getCity());
		insertVlu.setString(3, importRecord.getName());
		insertVlu.setInt(4, vluImport.getId());
		insertVlu.addBatch();
		deleteOldVlu.clearParameters();
		deleteOldVlu.setString(1, importRecord.getState());
		deleteOldVlu.setString(2, importRecord.getCity());
