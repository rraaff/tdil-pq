...
public class GoToChangeDealerAction extends AbstractAction {
	
	@Override
	protected ActionForward basicExecute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ChangeDealerForm aForm = (ChangeDealerForm)form;
		try {
			aForm.reset();
			WebsiteUser user = (WebsiteUser)getLoggedUser(request);
			aForm.setUser(user);
			aForm.setIdDealer(Integer.parseInt(request.getParameter("idDealer")));
	...
	private static Logger getLog() {
		return LoggerProvider.getLogger(GoToChangeDealerAction.class);
	}
