package com.tdil.ljpeugeot.dao.impl.sqlserver;

import com.ibatis.sqlmap.client.SqlMapClient;
import com.tdil.ljpeugeot.dao.BlobDataDAO;
import com.tdil.ljpeugeot.model.BlobData;
import com.tdil.ljpeugeot.model.BlobDataExample;
import java.sql.SQLException;
import java.util.List;

public class BlobDataDAOImpl implements BlobDataDAO {

	/**
	 * This field was generated by MyBatis Generator. This field corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	private SqlMapClient sqlMapClient;

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	public BlobDataDAOImpl(SqlMapClient sqlMapClient) {
		super();
		this.sqlMapClient = sqlMapClient;
	}

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	public int countBlobDataByExample(BlobDataExample example) throws SQLException {
		Integer count = (Integer) sqlMapClient.queryForObject("dbo_BLOB_DATA.countByExample", example);
		return count;
	}

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	public int deleteBlobDataByExample(BlobDataExample example) throws SQLException {
		int rows = sqlMapClient.delete("dbo_BLOB_DATA.deleteByExample", example);
		return rows;
	}

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	public int deleteBlobDataByPrimaryKey(Integer id) throws SQLException {
		BlobData _key = new BlobData();
		_key.setId(id);
		int rows = sqlMapClient.delete("dbo_BLOB_DATA.deleteByPrimaryKey", _key);
		return rows;
	}

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	public Integer insertBlobData(BlobData record) throws SQLException {
		Object newKey = sqlMapClient.insert("dbo_BLOB_DATA.insert", record);
		return (Integer) newKey;
	}

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	public Integer insertBlobDataSelective(BlobData record) throws SQLException {
		Object newKey = sqlMapClient.insert("dbo_BLOB_DATA.insertSelective", record);
		return (Integer) newKey;
	}

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	@SuppressWarnings("unchecked")
	public List<BlobData> selectBlobDataByExampleWithBLOBs(BlobDataExample example) throws SQLException {
		List<BlobData> list = sqlMapClient.queryForList("dbo_BLOB_DATA.selectByExampleWithBLOBs", example);
		return list;
	}

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	@SuppressWarnings("unchecked")
	public List<BlobData> selectBlobDataByExampleWithoutBLOBs(BlobDataExample example) throws SQLException {
		List<BlobData> list = sqlMapClient.queryForList("dbo_BLOB_DATA.selectByExample", example);
		return list;
	}

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	public BlobData selectBlobDataByPrimaryKey(Integer id) throws SQLException {
		BlobData _key = new BlobData();
		_key.setId(id);
		BlobData record = (BlobData) sqlMapClient.queryForObject("dbo_BLOB_DATA.selectByPrimaryKey", _key);
		return record;
	}

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	public int updateBlobDataByExampleSelective(BlobData record, BlobDataExample example) throws SQLException {
		UpdateByExampleParms parms = new UpdateByExampleParms(record, example);
		int rows = sqlMapClient.update("dbo_BLOB_DATA.updateByExampleSelective", parms);
		return rows;
	}

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	public int updateBlobDataByExampleWithBLOBs(BlobData record, BlobDataExample example) throws SQLException {
		UpdateByExampleParms parms = new UpdateByExampleParms(record, example);
		int rows = sqlMapClient.update("dbo_BLOB_DATA.updateByExampleWithBLOBs", parms);
		return rows;
	}

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	public int updateBlobDataByExampleWithoutBLOBs(BlobData record, BlobDataExample example) throws SQLException {
		UpdateByExampleParms parms = new UpdateByExampleParms(record, example);
		int rows = sqlMapClient.update("dbo_BLOB_DATA.updateByExample", parms);
		return rows;
	}

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	public int updateBlobDataByPrimaryKeySelective(BlobData record) throws SQLException {
		int rows = sqlMapClient.update("dbo_BLOB_DATA.updateByPrimaryKeySelective", record);
		return rows;
	}

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	public int updateBlobDataByPrimaryKeyWithBLOBs(BlobData record) throws SQLException {
		int rows = sqlMapClient.update("dbo_BLOB_DATA.updateByPrimaryKeyWithBLOBs", record);
		return rows;
	}

	/**
	 * This method was generated by MyBatis Generator. This method corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	public int updateBlobDataByPrimaryKeyWithoutBLOBs(BlobData record) throws SQLException {
		int rows = sqlMapClient.update("dbo_BLOB_DATA.updateByPrimaryKey", record);
		return rows;
	}

	/**
	 * This class was generated by MyBatis Generator. This class corresponds to the database table dbo.BLOB_DATA
	 * @mbggenerated  Sat Feb 01 15:35:30 ART 2014
	 */
	protected static class UpdateByExampleParms extends BlobDataExample {
		private Object record;

		public UpdateByExampleParms(Object record, BlobDataExample example) {
			super(example);
			this.record = record;
		}

		public Object getRecord() {
			return record;
		}
	}
}